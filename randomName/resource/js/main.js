...
//可修改参数
var chooseDelay = 80; //特效：名字闪动时间间隔（单位：ms）
var chooseTimes = 30; //特效：名字闪动次数
...
function run(times) {
    chooseName(times);
    times--;
    $('#progressbar').width((((chooseTimes - times+1) / chooseTimes) * 100).toString() + '%');
    if (times >= 0) {
        setTimeout(function() {
                run(times);
            },
        ...
        setTimeout(function() {
            afterChoose();
            $("#div-running").css("display","none");
            $("#div-start").css("display","");
        },0);
    }
}
